...
impl State {
    pub async fn new(window: Arc<Window>) -> anyhow::Result<Self> {
        // Descriptor for WGPU instance (default for now)
        let wgpu_instance_descriptor = wgpu::InstanceDescriptor::default();

        // The wgpu_instance is a handle to our GPU
        // It is responsible for creating the adapter and the surface
        ...
        let request_adapter_options = wgpu::RequestAdapterOptions {
            // See https://docs.rs/wgpu/latest/wgpu/enum.PowerPreference.html
            // Default will favor the low power settings
            power_preference: wgpu::PowerPreference::default(),

            // this field is provided &surface so that wgpu can use it to determine which adapter can present this surface
        ...
        let adapter = wgpu_instance.request_adapter(&request_adapter_options).await?;

        let device_descriptor = wgpu::DeviceDescriptor {
            label: None,

            // allows us to specify what extra features we want. https://docs.rs/wgpu/latest/wgpu/struct.Features.html
            required_features: wgpu::Features::empty(),

            // describes the limit of certain types of resources that we can create
            required_limits: wgpu::Limits::default(),

            // provides the adapter with a preferred memory allocation strategy, if supported. https://wgpu.rs/doc/wgpu/enum.MemoryHints.html
