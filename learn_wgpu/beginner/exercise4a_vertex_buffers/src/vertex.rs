#[repr(C)]
#[derive(
    Copy,                       // the Vertex to be Copy so we can create a buffer with it
    Clone, 
    Debug, 
    bytemuck::Pod,              // Pod indicates that our Vertex is "Plain Old Data", and thus can be interpreted as a &[u8]
    bytemuck::Zeroable          // Zeroable indicates that we can use std::mem::zeroed()
)]
pub struct Vertex {
    position: [f32; 3],         // represents the x, y, and z of the vertex in 3d space
    color: [f32; 3],            // represents the red, green, and blue values for the vertex
}
